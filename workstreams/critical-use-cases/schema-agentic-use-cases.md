...
This is a starting point, not a closed spec. If a real workflow doesn't fit (a missing primitive, a missing value, a different way to draw a line), propose the change. See [Open questions](#open-questions) for what's already on the table. 
...
- [Classification](#classification): the primitives and how they relate
...
**Browsing classified use cases:** go to the [Use Case Inventory](./use-case-inventory.md) and jump to a category. Every row is tagged with the primitives defined here.
...
**Twelve columns total. Five are proposed as primitives; the rest describe or support a row.**

A primitive marks a structural choice: something a workflow's design settles one way or another, not just a fact true of one implementation. Proposed primitives:
...
The other seven columns aren't primitives:

- **Duration** is measured, not chosen: how long one implementation happened to take. Two systems with identical primitive values can land at very different durations.
- **Use Case, Description, Tools & Systems** identify and describe the row.
- **Key Failure Mode, Preconditions, Notes** carry rationale and evidence.
...
The five are meant to compose: pick a value for each, and you have the control-flow skeleton a workflow needs. That's not a guarantee every combination is valid (see the `Atomic action` and `Single-agent, orchestrated` constraints above), and it's not a claim the five fully specify a system. Tool integrations and task logic still sit outside them, currently as free text in Supporting fields. Whether that should change is an [open question](#open-questions).
...
| Primitive | Description | Allowed values |
|---|---|---|
...
### Should `Tools & Systems` be a primitive?
