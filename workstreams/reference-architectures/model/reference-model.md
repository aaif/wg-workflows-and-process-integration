...
The reference model is the "big picture": the logical anatomy of a workflow run, independent of any engine or framework. Everything else in `model/` refines a part of it.
...
"Who is in control" is too imprecise for review. WERA separates control into five authorities that MAY be held by different actors for the same step ([INV-006](../foundations/architecture-invariants.md)).
...
A step result may be a **proposal** rather than an **accepted transition**. The workflow accepts a proposal only after the relevant validation, policy, evidence, and authority checks succeed ([INV-003](../foundations/architecture-invariants.md)).
