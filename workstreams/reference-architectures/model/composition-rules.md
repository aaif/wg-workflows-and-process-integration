...
Composition rules (`CR-###`) constrain how primitives may be combined into a valid workflow. They turn the [invariants](../foundations/architecture-invariants.md) and [design principles](../foundations/design-principles.md) into concrete "you may / you must / you must not compose it this way" guidance. Rules use RFC 2119 language.

## Rules

### CR-001 — Deterministic baseline documented

The design MUST document the deterministic baseline and the reason each nondeterministic primitive (`AIN`, `CLS`, `GEN`, `REC`, `PLN`, `TSL`, `ABR`) is needed ([DP-02](../foundations/design-principles.md)).

### CR-002 — Semantic output is validated before use

Any `AIN`/`CLS`/`GEN`/`REC`/`PLN` output that drives a transition or effect MUST be followed by `DVL`, `POL`, or `APR` before it is accepted ([INV-003](../foundations/architecture-invariants.md)).

### CR-003 — No effect without authorisation

`TRW`, `TXN`, `THI` MUST be preceded by `POL` (and `APR` where impact requires) in the same accepted path ([INV-009](../foundations/architecture-invariants.md)).

### CR-004 — Effect proposer ≠ sole authoriser

The primitive that proposes an effect MUST NOT be the only authoriser of it ([INV-007](../foundations/architecture-invariants.md)).

### CR-005 — Writes carry idempotency

Every `TRW`/`TXN`/`THI` MUST be paired with `IDM`, and ambiguous results MUST route to `RCP`, not a bare `RTY` ([INV-011](../foundations/architecture-invariants.md)).
...
Any `WAI` that can outlive the process MUST be backed by `CHK` and durable state (`DUR2`+) ([DP-08](../foundations/design-principles.md)).
...
Every `DLG` MUST declare allowed tools/data, maximum effect level, budgets, exit states, and state owner before execution ([INV-008](../foundations/architecture-invariants.md)).
...
Every `APR` MUST bind to an immutable artifact version/digest; a material change MUST invalidate it ([INV-013](../foundations/architecture-invariants.md)).

### CR-013 — Untrusted input is gated

`INP`/`RET` of untrusted content MUST pass input controls before any semantic primitive consumes it ([INV-018](../foundations/architecture-invariants.md)).

### CR-014 — One system of record

Accepted state MUST be written through `EVH`/`CHK` to a single system of record; `SME`/`DME`/`TRC` MUST NOT be the sole record ([INV-002](../foundations/architecture-invariants.md)).

### CR-015 — Terminal outcome required

Every run path MUST end in `FSK` with a terminal outcome from the closed set ([INV-005](../foundations/architecture-invariants.md)).

### CR-016 — Effect level matches controls

The controls attached to an effect MUST match its `effect_level` coordinate (e.g. `EF4` requires `APR`/segregation) ([DP-06](../foundations/design-principles.md)).

### CR-017 — Cross-WG concerns via boundaries

Security, identity, observability, governance, and accuracy needs MUST be expressed as `XB-##` references, not new local primitives ([INV-015](../foundations/architecture-invariants.md)).

### CR-018 — Budgets on dynamic regions

Any region containing `PLN`/`DLG`/`ABR`/`LOP` MUST declare a `CST`/budget bound ([DP-09](../foundations/design-principles.md)).
...
A composed workflow MUST use only codes present in the [registry](../descriptor/registry.yaml); a new concept requires justification ([DP-10](../foundations/design-principles.md), [INV-017](../foundations/architecture-invariants.md)).
