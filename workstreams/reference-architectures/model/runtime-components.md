...
Primitives are logical capabilities; **runtime components** (`RC-##`) are the logical services that host them. They are deliberately product-neutral ([INV-016](../foundations/architecture-invariants.md)): a single product may implement several components, and one component may be split across services.
