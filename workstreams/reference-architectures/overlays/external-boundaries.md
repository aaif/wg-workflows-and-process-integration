...
External boundaries (`XB-##`) are concerns essential to real workflows but **owned by other AAIF working groups**. WERA names the boundary, states what a workflow must hand across it, and stops there — it does not restate or fork the owning group's requirements ([INV-015](../foundations/architecture-invariants.md), [CR-017](../model/composition-rules.md)).
...
|---|---|---|---|---|
| XB-01 | Security & privacy | Security & Privacy | Trust-boundary declarations, untrusted-input tagging ([INV-018](../foundations/architecture-invariants.md)), least-privilege effect credentials | Define threat models, crypto, or attack-surface analysis |
