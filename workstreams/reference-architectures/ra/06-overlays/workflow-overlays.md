...
- **Requirements:** approver identity and role verified; approval binds an exact version/digest ([INV-013][architecture-invariants]); explicit approve/reject/request-change/timeout outcomes ([INV-014][architecture-invariants]); no self-approval ([INV-007][architecture-invariants]); recheck immediately before execution.
- **Evidence:** approval record, digest, presented-evidence manifest, approver authority, execution correlation.
- Realised by pattern [WP07](../03-patterns/wp07-human-supervised-action.md).
...
- **Requirements:** proposer holds no unrestricted target credentials ([INV-010][architecture-invariants]); explicit versioned payload ([INV-012][architecture-invariants]); proposer ≠ sole authoriser ([INV-007][architecture-invariants]); maximum effect level declared before execution.
...
- **Adds:** `EVH`, `CHK`, `AUD`.
- **Requirements:** accepted transitions recorded to a single system of record ([INV-002][architecture-invariants], [CR-014](../02-architecture-model/composition-rules.md)); distinct from observability signals, which are `XB-03`.
...
- **Trigger:** a dynamic region (`PLN`/`DLG`/`ABR`/`LOP`) needs bounded cost/time/steps ([DP-09](../01-foundations/design-principles.md), [CR-018](../02-architecture-model/composition-rules.md)).
- **Adds:** `CST`, `TMO`, `CAN`.
