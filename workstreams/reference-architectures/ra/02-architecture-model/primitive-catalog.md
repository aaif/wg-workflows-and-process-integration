...
The machine-readable list is in [registry.yaml](../../descriptor/registry.yaml). Adding a new primitive requires justification per [DP-10](../01-foundations/design-principles.md).
...
- **INP — Input intake.** Accept and canonicalise starting data and provenance. *Deterministic.* Controls: schema, classification tagging, untrusted-content handling ([INV-018][architecture-invariants]).
...
- **SME — Session memory.** Hold within-run working memory. *Either.* Controls: not a system of record ([INV-002][architecture-invariants]).
...
- **RTY — Retry.** Re-attempt on a retry class. *Deterministic.* Controls: retry class, idempotency ([INV-011][architecture-invariants]).
...
- **APR — Human approval.** Human authorises an exact version/action. *Human.* Controls: identity, exact-version binding ([INV-013][architecture-invariants]).
...
- **DLG — Delegate goal.** Hand a bounded sub-goal to an agent or sub-run under an explicit contract. *Either.* Controls: goal, allowed tools/data, max effect level, budgets, exit states, state owner ([INV-008][architecture-invariants]).
