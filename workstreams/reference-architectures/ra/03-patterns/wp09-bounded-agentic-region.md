...
delegation)](../01-foundations/architecture-invariants.md) checkable rather than
...
- **Tool allow-list** — the exact `TRO`/`TRW`/`THI` capabilities available inside,
  with credentials constrained per [INV-010](../01-foundations/architecture-invariants.md).
- **Budget** — step, token, wall-clock, and effect-count ceilings, owned by
  [OV-06][workflow-overlays]; the first ceiling hit forces `FSK`.
- **Effect ceiling** — the highest `EFn` the agent may cause unescalated; anything
  above escalates to [OV-01][workflow-overlays] rather than proceeding.
...
reconciled via [OV-03][workflow-overlays] (discard or compensate partial
...
[EP4](../04-profiles/ep4-agent-directed.md).
