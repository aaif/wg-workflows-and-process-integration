...
| CL1 | Invariant conforming | Meets all applicable [invariants](../01-foundations/architecture-invariants.md) (`INV-###`) and the [composition rules](../02-architecture-model/composition-rules.md) (`CR-###`). |
| CL2 | Overlay conforming | `CL1` plus every triggered [workflow overlay](../06-overlays/workflow-overlays.md) (`OV-##`) is applied, and every touched [external boundary](../06-overlays/external-boundaries.md) (`XB-##`) is declared. |
