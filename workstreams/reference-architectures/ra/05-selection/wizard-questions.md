...
An ordered question set that drives pattern and profile selection. Each question maps answers to codes in the [classification coordinate](../02-architecture-model/classification.md) or directly to [patterns](../03-patterns/README.md), [profiles](../04-profiles/README.md), and [overlays](../06-overlays/workflow-overlays.md). The machine-readable form is [selection-logic.yaml](selection-logic.yaml).
...
Name the single system of record ([INV-002][architecture-invariants]). Confirms `EVH`/`CHK` target.
...
- `yes` → input controls before any semantic step ([INV-018][architecture-invariants]); declare `XB-01`.
...
`IM0`…`IM4` → sets the [readiness](../07-readiness/readiness-tiers.md) floor.
...
- `yes` → `DLG` with a bounded contract ([INV-008][architecture-invariants]), `OV-06` budgets; `WP09`/`WP10`.
