...
| action-authorisation | `workflow` (+ `human`) | Effects are gated by rules; risky ones escalate to a human via [WP07][readme]. |
| execution | `workflow` + `single_agent` | The model runs inside an engine-invoked step ([WP01/WP02][readme]). |
...
Authority stays with `workflow` except where explicitly and narrowly shared, honouring INV-006 and [DP-04 (explicit authority)](../01-foundations/design-principles.md).
...
- Patterns: [WP00 deterministic baseline](../03-patterns/wp00-deterministic-baseline.md) for the spine, [WP02 recommend+adjudicate][readme] at the model step, [WP07 human-supervised action][readme] on risky effects — all wrapped in a [WP08 durable envelope][readme].
- Also common: [WP01 bounded model step][readme], [WP04 generate-validate-repair][readme], [WP06 model-assisted exception][readme].
...
- Approval drift: an approval captured against one model output is reused after the output changed — mitigate with [INV-013 (approval binds version)](../01-foundations/architecture-invariants.md) and [OV-01 human approval][workflow-overlays].
- Unbounded model steps: missing token/time budgets ([OV-06 budget/quota][workflow-overlays]) let a single step stall the run.
- Missing provenance: model output not written to [execution history](../01-foundations/core-concepts.md) ([OV-07][workflow-overlays]) breaks auditability.

## Example

The [invoice processing example](../../examples/invoice-processing/README.md) is the canonical EP2 run. A durable workflow ([WP08][readme]) receives an invoice, runs a model step to extract and recommend coding/approval routing ([WP02][readme]), gates high-value or low-confidence invoices to a human approver ([WP07][readme], `APR`), then posts and archives deterministically ([WP00](../03-patterns/wp00-deterministic-baseline.md)). The engine owns control-flow, decision, and state throughout; the model only ever proposes.

<!-- link definitions -->
[readme]: ../03-patterns/README.md
