...
Full treatment: [reference model](../02-architecture-model/reference-model.md).
...
An **effect** is an externally visible action. Effects are classified by level ([`EF0`–`EF4`][classification]) and are protected: authorisation, execution, idempotency, and confirmation are explicit and separated from reasoning.

## Actor topology

The arrangement of actors in a run — `none`, `single_agent`, `bounded_agentic_region`, `multi_agent`, `cross_runtime` — is **one classification axis**, not the organising principle of the architecture. See [classification][classification].
...
Technical completion and business acceptance are distinguishable. A run may finish technically while producing a business outcome such as rejected, abstained, escalated, or manually disposed. The closed set of terminal outcomes is defined in [classification][classification].

<!-- link definitions -->
[classification]: ../02-architecture-model/classification.md
