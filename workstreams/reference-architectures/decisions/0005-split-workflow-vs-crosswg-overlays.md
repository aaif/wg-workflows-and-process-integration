...
- **External boundaries** (`XB-##`) — owned by other WGs: security, identity, observability, governance, accuracy — **referenced, not redefined** ([INV-015](../foundations/architecture-invariants.md)).
...
Patterns and descriptors reference `XB-##` rather than embedding another group's requirements. The [external-boundaries](../overlays/external-boundaries.md) document is a boundary map, not a spec.
