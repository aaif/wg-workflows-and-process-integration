...
The architect explicitly requires **agent-directed control-flow** for this variant: the
agent owns the whole run toward the green gate (`EP4`) rather than operating only inside
a workflow-owned bounded region (`EP3`). This intentionally overrides the usual
least-agentic preference (`DP-01`) to explore the greater autonomy and its cost. The
solution must retain — and strengthen where necessary — the envelope, protected-effect,
budget, durability, audit, and escalation safeguards; it must not grant merge, deploy,
production, or authoritative-repository access.
