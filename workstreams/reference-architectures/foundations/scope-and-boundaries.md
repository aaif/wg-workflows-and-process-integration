...
Several concerns are essential to real workflows but are owned by **other AAIF working groups**. WERA references them as [external boundaries](../overlays/external-boundaries.md) (`XB-##`) and does not restate their requirements:
...
- Where exactly is the boundary between a "bounded agentic region" and "multi-agent" for classification purposes? (see [classification](../model/classification.md))
