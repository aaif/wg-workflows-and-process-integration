...
3. [selection-logic.yaml](../selection/selection-logic.yaml) — execute selection deterministically.
...
- every code used exists in [registry.yaml](../descriptor/registry.yaml) ([INV-017](../foundations/architecture-invariants.md));
- the descriptor validates against the schema;
- all applicable [invariants](../foundations/architecture-invariants.md) and [composition rules](../model/composition-rules.md) hold (target at least `CL1`);
- no cross-WG concern was redefined locally ([INV-015](../foundations/architecture-invariants.md));
- the least-agentic viable composition was chosen ([DP-01](../foundations/design-principles.md)).
