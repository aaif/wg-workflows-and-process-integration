...
| state | `workflow` | The engine is the single [authoritative state owner](../foundations/architecture-invariants.md) (INV-002). |
...
- Patterns: [WP00 deterministic baseline](../patterns/wp00-deterministic-baseline.md), often wrapped in [WP08 durable envelope](../patterns/README.md).
...
- Over-fitting the happy path: missing durable retries/compensation ([OV-04 durable wait](../overlays/workflow-overlays.md)) turns transient faults into stuck runs.
