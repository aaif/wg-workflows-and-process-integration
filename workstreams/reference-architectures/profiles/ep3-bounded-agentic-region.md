...
The defining moves are `DLG` (delegate goal) at entry and a result/`HND` back at exit, realising [WP09 bounded agentic region](../patterns/README.md) under [INV-008 (bounded delegation)](../foundations/architecture-invariants.md) and [DP-09 (bound dynamic regions)](../foundations/design-principles.md).
...
- A single EP2 run commonly *contains* an EP3 region: the engine reaches a step, delegates a bounded goal, and resumes deterministically with the result. Budgets come from [OV-06 (budget/quota)](../overlays/workflow-overlays.md); the plan-and-execute inside typically follows [WP05](../patterns/README.md).
...
- Who authorises effects the agent proposes inside the region that exceed the pre-negotiated envelope — silent denial, or escalation to a human ([OV-01](../overlays/workflow-overlays.md))?
...
- How is the region's internal `PLN`/`TSL` activity recorded in [execution history](../foundations/core-concepts.md) without leaking non-authoritative scratch state into the authoritative store?
